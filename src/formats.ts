...
export function formatNestings(text: string, config: any): string {
  ...
  try {
    ...
    codeFragments = text.split(CRLF); // split code by line (behält ggf. leeres letztes Fragment wenn CRLF am Ende)
    ...
    for (let i = 0; i < codeFragments.length; i++) {
      ...
      if (!isLast) {
        ...
        if (codeFragments[i] !== "") {
          buf += codeFragments[i];
          if (hadFinalCRLF) {
            buf += CRLF; // Erhalte genau ein CRLF am Ende falls vorher vorhanden.
          }
    ...
    if (modified > 0) {
      ...
      if (txt[i] === LF) {
        if (inString) {
          ...
          log(
            "Error",
            `Error @ Line ${LineCount} at Column ${ColumnCount} -> no closed string detected!`
          );
          log("info", buf);
      ...
      if (!inComment && txt[i] === "}") {
        log(
          "Error",
          `Error @ Line ${LineCount} at Column ${ColumnCount} -> closed comment bracket witout Open comment bracket!`
        );
        log("Error", buf);
...
function CheckCRLForWhitespace(s: string): boolean {
  ...
  return FORMATS.concat(SINGLE_OPERATORS, DOUBLE_OPERATORS, TRENNER).some(
    (item) => s === item
  );
}
